(** * RevExtractP.v — verified executable interpreter *with parameters*

    Like [RevExtract.v], but for [RevProc.v]'s language: procedures take
    reference parameters.  A call evaluates the body renamed by the
    formals->actuals binding ([rename (argsubst (fst (Γ p)) args) (snd (Γ p))]),
    exactly matching [RevProc]'s [E_Call]/[E_Uncall].  Proved sound vs.
    [RevProc.exec] and extracted to OCaml ([janus_param.ml]); this is the
    interpreter the differential harness uses to cover the real (procedure-using)
    fixtures. *)

From Stdlib Require Import ZArith Bool Extraction.
Require Import RevProc.
Import RevProc.
Open Scope Z_scope.
...
Extraction "janus_param.ml" run invert.
