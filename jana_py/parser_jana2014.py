"""Jana 2014 parser based on upstream `Jana.Parser`.

This mode models the non-basic parser from the original Haskell project:
  - `procedure`-based syntax with explicit parameter lists
  - `procedure main()` with typed variable declarations at the top
  - `call` / `uncall` with explicit argument lists
  - `<=>` swap and `+=` / `-=` / `^=` updates
  - `if ... then ... fi expr` and `from ... until ...`

It intentionally differs from `jana2014basic`, which tracks `ParserBasic.hs`.
"""
...
class Parser:
  ...
  def _looks_like_expr(self) -> bool:
    ...
    if token.kind == "KW" and token.value in {
      "if", "from", "push", "pop", "local", "delocal", "call", "uncall",
  ...
  def _stmt_requires_semicolon(self, stmt) -> bool:
    return not isinstance(
      stmt,
      (IfStmt, FromStmt, IterateStmt, LocalStmt, BareLocalStmt, AncillaBlockStmt),
    )
