...
Emits `procedure`-based syntax (no braces/semicolons, `if..then..fi`,
`from..until`) so that `-i` output re-parses under the jana2014-family and
janus1982-family dialects. The C-style (janus2026) formatter lives in
`format.py`; `cli.py` picks one based on `--std`.
"""
...
def format_struct_def(struct_def: StructDef) -> str:
  # Field lists are C-style (`;`-terminated), matching the dialect parsers.
