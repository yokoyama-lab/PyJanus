...
### [x] 40. 被覆率と決定率を測る（項目39 の後。**着手前に予測を書いてから測る**）
...
- 2026-08-09 項目40 完了、**キューが空になった**（§21）。
  **被覆率 unsupported 67 → 57（+10本が断片入り）、proved 20 → 26、refuted は 0 のまま。**
  stack を使う実物3本（`stack_uncall_c` / `run_length_enc_stack_c` / `gcd_g`）が
  **2 invariants とも proved ＝ 無条件**で通った。`bound`（上限の内側だけ）は 0 本なので
  **深さ 8 で足りており、`D` を上げる理由が無い**。
  **予測と実測**（commit 9ade339 で凍結）: 断片入り 12→**10**（近い）、
  compile-timeout 5→**1**、決着 3→**6**、無条件 1〜2→**3**。
  **5回連続で外した**（29・34・35・36・40）が、今回は実測の方が良い。外れの向きが
  揃っていないので、結論は「甘い」でも「辛い」でもなく**単に当たらない**。
  **測定の途中で自分の誤検出を2件出し、どちらもコーパス走査が捕まえた**:
  (a) `size(配列)` を stack 専用にして `perm_to_code_c` を refuted にした
  （直した結果その1本が proved に）。(b) **短絡の右側の義務を無条件に積んでいた**ため
  `run_length_enc_stack_c` を refuted にした——これは **stack より前から在ったバグ**で、
  短絡の右に除算を置くプログラムがコーパスに無かったので誰も踏んでいなかった。
  副産物: `ext_gcd_g` が **IC3 の「断念」**として返った。§5.10 で「断念は1本も無い」と
  書いた状況が変わったので §5 に注記した。
  **キューが空になったのでループを止める。** 残るのは項目37（人の判断）のみ。
