...
版でも同じ）。300 秒で打ち切ると 87% で止まる。**内側ループではこれを外して回す**:
...
### [ ] 31. `run-length-enc` を1文ずつ削って proved に転じる点を探す
...
### [ ] 32. `glaisher` で項目31 と同じことをする

**前提**: 項目31 が (a) で終わった場合のみ着手する。(b)（空振り）だったら
**この項目は skip して `[x] 見送り` と書く**——1本目で空振りしたやり方を2本目で
繰り返す理由がない。
