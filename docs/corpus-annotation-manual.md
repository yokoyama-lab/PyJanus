...
`tests/jana2014/fixtures/examples/` に、Janus で書かれたサンプルプログラムが **101本**
入っています。ソート、最大公約数、フィボナッチ、圧縮など、教科書に出てくる
アルゴリズムを「逆実行できる形」で書き直したものです。

> **2026-08-06 追記 — 仕事の中身が変わりました。**
> 下に書いてある「全数に注釈を付ける」作業は、**LLM が全数を生成して完了済み**です
...
101本すべてに、**同じアルゴリズムを Python で書き直した参照実装**が
...
`report` が `annotated 101/101` と出て、`check` が `0 failed` で終われば
準備完了です。
...
101本すべてで `passed` になるはずです。**あなたが `@keep` を書き間違えると、
このテストではなく `check` が落ちます**（`check` があなたの `@keep` を、
このテストが参照実装の `GARBAGE` を、それぞれ同じ実行結果に照らすためです）。

### 1.4 見本を読む

**手で書かれた見本が4本**あります（残りは LLM 生成）。**作業を始める前に必ずこの4本を読んでください。**
...
**ゴミがあれば末尾は `_g`、無ければ `_c` です**（101本すべてがどちらかで終わります。
ゴミありが33本、ゴミ無しが68本）。
...
> **2026-08-07 現在の残量**: 下の4つのうち **10.3 は完了、10.1 はほぼ完了**です。
> 実際に残っているのは **10.4（`@oracle` 97本・約15h）** と
> **10.2（`@technique` の1論点・約2h）**、それに 10.1 の判断分（約2h）。
> **合計およそ 19 時間**で、当初見積の 33〜35h から縮んでいます。
> 予算80h の残りは、`docs/textbook-programs-plan.md` の
> **新しいプログラムを足す作業**（§11）に充てるのが自然です。

101本の注釈は LLM が全数生成しました（手書きの見本4本を除く）。**形式・数値・ゴミの判定は機械が検査済み**で、
...
ヒープの並び）は埋まりました。**101本すべてで答え全体が独立に再現されています。**
...
### 10.4 `@oracle` を書く（97本・ここが本編）

`@oracle` が付いているのは4本だけです。残り97本に、**参照実装を見ずに**、
§2 手順5 の要領で書いてください。
...
## 11. その次 — 新しい可逆プログラムを足す

フェーズ A〜E（注釈・命名・参照実装・被覆マトリクス・エラー fixture）は
**すべて完了済み**です。`docs/corpus-cleanup-plan.md` に残タスクはありません。

次の仕事は **corpus を増やすこと**で、手順は `docs/textbook-programs-plan.md` にあります。
1本足すときの流れは §2 とほぼ同じで、追加で次が要ります。

1. **重複確認** — `ls tests/jana2014/fixtures/examples/` と `grep -ri <キーワード>`
2. **技法を決めてから書く** — §2 手順5 の5分類のどれを見せる例にするか
3. **`tests/jana2014/reference/<名前>.py` を書く** — `expected()` と `GARBAGE`。
   **Janus を書き写さずアルゴリズムから書く**（`tests/jana2014/reference/README.md`）
4. **`_g` / `_c` を名前に付ける** — ゴミの有無はテストが判定する
5. **被覆マトリクスを再生成** — 忘れると `test_every_example_has_a_row` が落ちる
6. `docs/textbook-programs-plan.md` のバックログに1行

バックログに残っているのは Pearls 側の2件（**逆BWT のインプレース版**、
**双射BWT (BBWT)**、arXiv:2004.12590）で、どちらも重い題材です。
学部生に渡すなら、まず先生と題材を決めてください。
