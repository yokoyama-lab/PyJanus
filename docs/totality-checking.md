...
21本**（2026-08-05 実測。配列対応前は8本、配列対応後・構造体対応前は17本）。
残る76本の**最初にぶつかる**阻害要因:
...
持つが、それだけで弾かれるものは無くなった。次の壁は **stack（25本）** で、可変長の
状態という別種の作業になる。それ以外は横断的な小物の集まりで、上位3つ（stack を除く）
は定数引数12本・`iterate` 11本・`^=` 9本である。**多次元配列は射程外**（`coq/RevLowering.v`
に Cantor fold の単射性があるので入口はある）。
...
| `unsupported` | 92 | 断片外（うち25本が stack の宣言） |
...
**内訳は割り切れている**: 断片内 43本＝ examples 21本（proved 11 / unknown 10）
...
examples 97本のうち断片内は**21本**（配列対応前は8本、構造体対応前は17本）。
**証明できたのは11本**（`base_convert`, `cantor_pair`, `fall`, `fib`,
`injective_basics`, `injective_bennett`, `zagier`, `structs_scalar`,
`structs_nested`, `structs_flat_repass`, `structs_array`）で、残る10本は未決
（`injective_bwt_inverse` / `knapsack` / `sqrt` は IC3 が断念、他はタイムアウト）。
