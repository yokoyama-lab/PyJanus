...
**この 2 という数字の読み方を、2026-08-09 に訂正した。** 当初ここには
「深さ上限を置く方式では 2本しか入らない」と書いたが、**言い過ぎだった**。
`max_depth` と同じく **BOUND 位置**（上限を超えた実行はそこへ抜け、継続は落とす）を
使えば、**26本すべてが断片に入る**。2 というのは「入る本数」ではなく、
**上限を超えないことが静的に分かる本数**である。両者の違いは主張の強さに出る:

| | 本数 | 得られる主張 |
|---|---:|---|
| 深さ上限を静的に正当化できる | **2** | 無条件（`pc != ERR`） |
| 上限つきで模型化できる | **26** | **上限を超えない実行についてのみ**。ただし `pc != BOUND` も証明できれば無条件になる |

**§6 の但し書きがそのまま効く**——「証明は上限に到達しない実行についてのみ」。
つまり設計判断は「効くか効かないか」ではなく、**上限つきの条件つき結果を受け入れるか**
だった。2026-08-09 に先生が「可変長の符号化へ進め」と判断したので、
`docs/loop-queue.md` の項目38〜40 に落としてある。
...
## 19. stack の意味論（PyJanus で実測。2026-08-09・項目38）

符号化は意味論の写しである。このプロジェクトで見つかった不健全性は**いずれも意味論を
推測した箇所**だった（swap の別名・剰余モードの黙殺・出力文の読み飛ばし）。
そこで **stack を符号化する前に、全部インタプリタに走らせて固定した**。
以下はすべて実測で、推測は1つも含まない。

| # | 何を | 実測 | 符号化への含意 |
|---:|---|---|---|
| 1 | `push(x, s)` | `s` の頂に `x` が積まれ、**`x` は 0 になる** | 積むだけでなく**源を零化する**。忘れると可逆性が壊れる |
| 2 | `pop(x, s)` で `x ≠ 0` | **実行時エラー** `Can't pop to non-zero variable 'y'` | **ERR 辺が要る**（`x = 0` が守るべき義務） |
| 3 | 空の `s` への `pop` | **実行時エラー** `Can't pop from empty stack` | **ERR 辺が要る** |
| 4 | 空の `s` への `top` | **同じエラー** `Can't pop from empty stack` | `top` も空で落ちる。式なのに ERR を持つ |
| 5 | 非空の `s` への `top` | **消費しない**（`s = <7]` のまま） | 純粋な読み。`pop` と混同しない |
| 6 | `size(s)` / `empty(s)` | 要素数 / 空なら **1**（整数として使える） | 個数変数 `s__n` から直に出る |
| 7 | `uncall` | `call fill(s); uncall fill(s)` で `s = nil` に戻る | push と pop は互いに逆。**段数も対称**であるべき |
| 8 | 手続きへ渡す | **参照渡し**（`call fill(s)` の後 `s = <5]`） | 配列と同じ。仮引数を同じ実体に束縛する |
| 9 | 同じ stack を2つの仮引数へ | **通る**（`call two(s, s)` がエラーにならない） | **別名を弾いてはいけない**。スカラの `x <=> x` と違い、操作が文ごとに逐次なので違反にならない |
| 10 | `local stack t = nil` | 通る。`delocal` は空であることを要求する | 局所 stack は corpus の `reverse` が使う |

**9 が特に重要である。** 配列とスカラでは別名検査が要ったので、stack でも同じだろうと
書きかけたが、**実測すると通った**。仮に弾いていたら `reverse` のような正しい
プログラムを誤って拒否する——過去の不健全性とは逆向きの、しかし同じ根の誤りになる。

**2・3・4 が ERR 辺の全部である。** 上限超過（項目39 の BOUND）とは**別物**なので、
符号化では混ぜないこと——ERR は「このプログラムは落ちる」、BOUND は「この模型の射程外」。

## 20. stack の符号化（深さ上限つき。2026-08-09・項目39）

§19 で測った意味論をそのまま写した。設計は `max_depth` の先例に倣う。

### 20.1 表現

`stack s` を **深さ `_STACK_DEPTH`（既定 8）のセル ＋ 個数 `s__n`** に展開する。
可変添字の読み書きは配列の機構（§12）がそのまま使える。

| 構文 | 符号化 |
|---|---|
| `push(x, s)` | `s__n < D` なら セル`[s__n] := x`・`s__n += 1`・**`x := 0`**、`s__n = D` なら **BOUND へ** |
| `pop(x, s)` | 義務 `s__n > 0` と `x = 0`（どちらも **ERR**）、`x := セル[s__n-1]`・`s__n -= 1` |
| `top(s)` | 義務 `s__n > 0`（**ERR**）、`セル[s__n-1]` を読む（消費しない） |
| `size(s)` / `empty(s)` | `s__n` / `s__n = 0`。**整数としても条件としても**使える（`if !empty(s)` が corpus にある） |
| `stack` 仮引数 | 同じ `_Stack` を束縛＝**参照渡し**。別名は**弾かない**（§19 の 9） |
| `local stack t = nil` | 空の stack を作り、`delocal` は **空であることの義務**（ERR） |

**ERR と BOUND は別物である。** ERR は「このプログラムは落ちる」、BOUND は
「この模型が追うのをやめた」。押し過ぎは後者であって前者ではない。

### 20.2 判定の報告が両者を潰していた

深さ 8 に 9 回 push するプログラムを測ると、**`pc != 0`（ERR）は proved、
`pc != 1`（BOUND）は refuted**。ところが `Result.status` は全性質の**最弱**を取るので、
これが `refuted`＝「表明が破れる」として報告されていた。**誤った告発である。**

`nuxmv.Result.status_of(prop)` を足して性質ごとに聞けるようにし、
`tools/verify_corpus.py` に **`bound`** という分類を追加した——
「上限の内側では証明できたが、上限には到達しうる」。§6 の但し書きがそのまま名前になった。

### 20.3 自分で入れた不健全性を、error fixture が捕まえた

**符号化を書いた直後の測定で、`type-error-argument.ja` が `proved` になった。**
`stack` を `int` の仮引数に渡すプログラムで、PyJanus は
`Couldn't match expected type 'int'` で落ちる。**落ちるプログラムを安全と証明していた**——
このプロジェクトが4回見つけてきたのと同じ型の誤りを、自分で1つ足したことになる。

原因は仮引数の束縛にある形の検査が `tuple` と比べていて、`_Stack` はタプルではないので
**すり抜けた**こと。同じ根で他に4件出た:

| fixture | 直す前 | 直した後 |
|---|---|---|
| `type-error-argument.ja` | **proved（不健全）** | refuted |
| `type-error-swap.ja` | `model-error`（`_Stack` の repr が模型に漏れて nuXmv が読めない） | refuted |
| `type-error-empty.ja` / `-size` / `-top` | `unsupported`（断片外扱い） | refuted |

いずれも**実行時の型エラー**なので、正しくは ERR である。式の文脈では
「常に偽の義務」を積むことで ERR へ落とす（式は辺を直接張れないため）。

**error fixture の検出は 23 → 29 本に増えた。** `proved` も `model-error` も 0 本。
**この5件は、キューが「error fixture を毎回測れ」と決めていなければ全部見逃していた。**

### 20.4 変異注入

`push` の `s__n += 1` を落とすと、差分試験 6 件のうち **2 件が不一致になる**
（正常な push/pop と 非空 top が refuted に化ける）。検出力があることの確認。

### 20.5 いま入っているもの・いないもの

`stack_uncall_c.ja` は **proved**（ERR も BOUND も。つまり無条件）。
