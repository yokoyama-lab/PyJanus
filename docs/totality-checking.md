...
|---:|---:|---:|---:|
| 120 秒（この表） | 13 | 13 | — |
| **600 秒（5 倍）** | **11**（測定中に打ち切り） | **11** | **0** |
| 5400 秒（45 倍。`run_length_enc_c` のみ） | 1 | 1 | 0 |

**測った 11 本は1本も転じなかった。** 残る2本（`run_length_enc_c` / `sqrt_g`）は
600 秒での測定が未了である——ただし `run_length_enc_c` は **5400 秒でも unknown**
なので、600 秒で転じることはない。**未測定は `sqrt_g` の1本**。 つまり:
