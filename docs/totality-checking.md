...
**`unknown` は難しさではなく打ち切りだった。** 同じモデルが、80 秒の隣（**モデルは
1行しか違わない**）で 314 秒かかる。§5.7 の「モデルサイズは律速でない」を別の角度から
裏づけると同時に、§5 系の測定値そのものへの警告になっている——
**あの `unknown` 13本は 120 秒という予算の関数であって、問題の難しさの関数ではない。**
