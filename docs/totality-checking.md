...
**proved は 10 のまま動かなかった**。内訳が過不足なく一致するので、
**examples から `refuted` は1本も出ていない＝誤検出ゼロは保たれている**。
新たに `proved` になった error fixture も無い。

つまり配列対応で**ボトルネックが移った**: 「断片外だから測れない」から
「表現できるが決められない」へ。理由は §5.5。

### 5.1 検出側 — 17/17、誤検出 0

**断片内にある実行時エラー fixture 17本を全て検出した。**配列対応で加わった5本は
`alias-2`（配列を跨ぐ別名）、`alias-swap-1` / `alias-swap-2`（セルの自己 swap と、
添字が swap 対象を読む形）、`array-out-of-bounds`、`array-size-mismatch`。
下の表は配列対応前からの12本。
...
examples 97本のうち断片内は**17本**（配列対応前は8本）。**証明できたのは7本**
（`base_convert`, `cantor_pair`, `fall`, `fib`, `injective_basics`,
`injective_bennett`, `zagier`）で、残る10本は未決（`injective_bwt_inverse` /
`knapsack` / `sqrt` は IC3 が断念、他はタイムアウト）。
6本目の `base_convert` は符号化形式を `assign` に揃えて得たもの（§5.4 の追記）、
7本目の `injective_bennett` は零ストアを本当に1点にして得たもの（§5.8）で、
どちらもアルゴリズムの改善ではない。`fib.ja` は不変量が2本＝ERR だけでなく
...
- **スタック・構造体は未対応**（examples 97本のうち宣言に stack が31本・struct が13本）。
  配列は対応した（§4）が、決定率はまだ上がっていない（§5.5）。
...
1. ~~配列をスカラーへ展開する~~ → **2026-08-04 に実施**（§4・§5.5）。長さ未指定の引数は
   `N` を与えるまでもなく、インライン展開が実引数を持っているので解決した。残った課題は
   **決定率**で、§5.5 のとおり項の大きさが効いている。次はここを、§5.4 と同じやり方で
   詰める——`_BLOCK_CHARS` を決定率で振る、要素ごとの条件付き更新をより小さい形に書く、
   配列を `array 0..n-1 of integer` として渡して nuXmv 側の理論に任せる、の3案を測る。
