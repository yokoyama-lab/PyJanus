...
- `push(x, s)` は `x` を 0 にクリアし、`pop(x, s)` は `x == 0` を要求する。
  **`x` は左辺値でなければならない**（`push(1, s)` は不正）。`push` は値を移動して
  元を空にする操作なので置き場所が要る、というのが理由だが、決め手は反転で閉じている
  こと: `invert` は `push` ↔ `pop` を写すので、`push(1, s)` を許すと、その逆
  `pop(1, s)`（インタプリタが従来から拒否している）が書けない手続きができてしまう。
  リテラルを積みたいときは機械的に書き換えられる:
  `local int t = 1  push(t, s)  delocal int t = 0`。
  検査は `validate_program`（静的）で行い、インタプリタと C++ バックエンドは
  それぞれ独立にも拒否する（多層防御）。
